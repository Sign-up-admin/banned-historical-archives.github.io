...
const archive_path_resolver_ts_1 = require("./backend/archive-path-resolver.ts");
...
try {
    ...
    const archiveBasePath = (0, archive_path_resolver_ts_1.resolveArchiveBasePath)(0);
    console.log(`  archives0 基础路径: ${archiveBasePath}`);
    // 测试 2: 解析书籍元数据路径
    console.log('\n测试 2: 解析书籍元数据路径');
    const bookMetadataPath = (0, archive_path_resolver_ts_1.resolveBookMetadataPath)(0, '0a4');
    console.log(`  书籍 0a4 元数据路径: ${bookMetadataPath}`);
    // 测试 3: 解析文章文件路径
    console.log('\n测试 3: 解析文章文件路径');
    const articlePath = (0, archive_path_resolver_ts_1.resolveArticlePath)(0, '0a4', 'some-article-id');
