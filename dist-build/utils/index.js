...
exports.bracket_right = exports.bracket_left = void 0;
exports.md5 = md5;
exports.extract_pivots = extract_pivots;
exports.apply_patch_v2 = apply_patch_v2;
exports.apply_patch = apply_patch;
exports.ensure_two_digits = ensure_two_digits;
exports.crypto_md5 = crypto_md5;
 ...
 * ```
 */
function extract_pivots(s, part_idx) {
    const res = [];
    const exp = new RegExp(`${exports.bracket_left}\\d+${exports.bracket_right}`);
    while (true) {
 ...
 * ```
 */
function apply_patch_v2(parserResult, patch) {
    ...
    for (const i in parts) {
        ...
        if (patch.parts[i]) {
            ...
            if (!patch.parts[i].delete) {
                ...
                if (patch.parts[i].diff) {
                    ...
                    comment_pivots
                        ...
                        .forEach((i) => original_text_arr.splice(i.offset, 0, `${exports.bracket_left}${i.index}${exports.bracket_right}`));
                    const original_text_with_brackets = original_text_arr.join('');
                    final_text = new diff_match_patch_1.diff_match_patch()
            ...
            final_pivots.push(...comment_pivots
        ...
        for (const idx_from_0 in comments) {
            ...
            if (patch.comments[idx_from_1]) {
                if (patch.comments[idx_from_1].insertBefore)
                    final_comments.push(...patch.comments[idx_from_1].insertBefore.map((j) => j.text));
                ...
                if (patch.comments[idx_from_1].insertAfter)
                    final_comments.push(...patch.comments[idx_from_1].insertAfter.map((j) => j.text));
            }
            else {
    ...
    if (typeof patch.description === 'string') {
        if (patch.description.length) {
            newResult.description = new diff_match_patch_1.diff_match_patch()
 ...
 * ```
 */
function apply_patch(parserResult, patch) {
    const d = new diff_match_patch_1.diff_match_patch();
    ...
    Object.keys(patch.parts).forEach((i) => {
        ...
        comment_pivots
            .filter((i) => i.part_idx === idx)
            .sort((a, b) => b.index - a.index)
            .forEach((i) => text_arr.splice(i.offset, 0, `${exports.bracket_left}${i.index}${exports.bracket_right}`));
        ...
        const new_text = diff
            ...
            .map((i) => i[1])
            .join('');
        parserResult.comment_pivots = comment_pivots.filter((x) => x.part_idx !== idx);
    ...
    Object.keys(patch.comments).forEach((i) => {
        const idx = parseInt(i);
        const diff = d.diff_fromDelta(parserResult.comments[idx - 1], patch.comments[i]);
        const new_text = diff
            .filter((i) => i[0] !== -1)
            .map((i) => i[1])
            .join('');
...
function get_article_id(r) {
    const res = crypto_md5(JSON.stringify([
        r.title,
        r.dates
            .sort((a, b) => `${a.year || '0000'}-${ensure_two_digits(a.month)}-${ensure_two_digits(a.day)}` >
            `${b.year || '0000'}-${ensure_two_digits(b.month)}-${ensure_two_digits(b.day)}`
            ? 1
            : -1)
            .map((a) => `${a.year || '0000'}-${ensure_two_digits(a.month)}-${ensure_two_digits(a.day)}`),
        !!r.is_range_date,
