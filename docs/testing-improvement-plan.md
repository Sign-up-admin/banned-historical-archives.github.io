...
## 🎯 阶段一：紧急修复（1-2周）/ Phase 1: Urgent Fixes (1-2 weeks)

**目标**: 解决阻塞性问题，建立基础测试能力

**Goal**: Resolve blocking issues and establish basic testing capabilities

### 任务组 1.1: 测试框架升级 / Test Framework Upgrade

#### 任务 1.1.1: 升级 Vitest 到 4.0+ 🔴
- **优先级**: 🔴 高
- **预计时间**: 0.5 天
- **依赖**: 无
- **可并行**: 否
- **描述**: 
  - 升级 Vitest 从 0.19.1 到 4.0+
  - 升级 `@vitest/coverage-v8` 到对应版本
  - 验证所有现有测试在新版本下正常运行
- **验收标准**:
  - [ ] Vitest 版本 >= 4.0.0
  - [ ] 所有现有测试通过
...
- **描述**:
  ...
  - 更新 `test/setup.ts` 引入 `@testing-library/jest-dom`
...
- **执行步骤**:
  1. 编辑 `vitest.config.ts`，取消注释 `coverage` 配置
  2. 编辑 `test/setup.ts`，添加 `import '@testing-library/jest-dom'`
  3. 更新 `package.json` 中的 `test:coverage` 脚本

### 任务组 1.2: CI/CD 集成 / CI/CD Integration

#### 任务 1.2.1: 创建测试 Workflow 🔴
- **优先级**: 🔴 高
- **预计时间**: 1 天
- **依赖**: 1.1.1
- **可并行**: 否
...
### 任务组 1.3: 测试基础设施完善 / Test Infrastructure Enhancement

#### 任务 1.3.1: 完善 test/setup.ts ⚡
- **优先级**: 🟡 中
- **预计时间**: 0.5 天
- **依赖**: 1.1.2
- **可并行**: 是（与 1.2.2 并行）
...
### 阶段一并行执行图 / Phase 1 Parallel Execution Diagram
...
gantt
    ...
    section 框架升级
    升级Vitest           :crit, 1.1.1, 2024-01-01, 0.5d
    安装React测试库      :1.1.2, after 1.1.1, 0.5d
    更新配置文件        :1.1.3, after 1.1.2, 0.5d
    section CI/CD
    创建测试Workflow     :crit, 1.2.1, after 1.1.1, 1d
    集成构建流程        :1.2.2, after 1.2.1, 0.5d
    section 基础设施
    完善setup.ts        :1.3.1, after 1.1.2, 0.5d
    建立目录结构        :1.3.2, 2024-01-01, 0.5d
```

**并行执行策略 / Parallel Execution Strategy:**
- 任务 1.1.1 和 1.1.2 可以并行（安装不同依赖）
- 任务 1.2.2 和 1.3.1 可以并行（不同模块）
- 任务 1.3.2 可以与其他所有任务并行（独立任务）

---

## 🔧 阶段二：核心测试补充（2-4周）/ Phase 2: Core Test Coverage (2-4 weeks)

**目标**: 提高核心功能测试覆盖率到 >= 80%

**Goal**: Improve core function test coverage to >= 80%

### 任务组 2.1: 工具函数测试 / Utility Function Tests

#### 任务 2.1.1: md5 和 crypto_md5 函数测试 ⚡
- **优先级**: 🔴 高
- **预计时间**: 1 天
- **依赖**: 1.1.3
- **可并行**: 是
...
#### 任务 2.1.2: get_article_id 函数测试 ⚡
- **优先级**: 🔴 高
- **预计时间**: 1 天
- **依赖**: 1.1.3
- **可并行**: 是（与 2.1.1 并行）
- **描述**:
  - 为 `get_article_id` 函数编写完整测试
  - 测试各种输入情况
  - 验证输出格式和唯一性
- **验收标准**:
  - [ ] 测试覆盖所有代码路径
...
#### 任务 2.1.3: apply_patch_v2 函数测试完善 ⚡
- **优先级**: 🔴 高
- **预计时间**: 2 天
- **依赖**: 1.1.3
- **可并行**: 是（与 2.1.1, 2.1.2 并行）
- **描述**:
  - 完善现有 `apply_patch_v2` 测试
  - 添加更多边界条件测试
  - 添加错误处理测试
  - 验证所有 patch 操作类型
- **验收标准**:
...
- **可并行**: 是（与其他工具函数测试并行）
- **描述**:
  - 为 `utils/index.ts` 中其他函数编写测试
  - 包括：`bracket_left`, `bracket_right`, `format_date` 等
  - 每个函数至少 3-5 个测试用例
- **验收标准**:
  - [ ] 所有导出函数都有测试
  - [ ] 每个函数覆盖率 >= 80%
